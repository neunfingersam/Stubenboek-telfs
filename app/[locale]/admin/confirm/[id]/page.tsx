    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-cream px-4">
      <div className="max-w-md w-full rounded-2xl bg-white p-8 shadow-lg">
        <h1 className="font-serif text-3xl font-bold text-alpine mb-6">
          {already ? 'Bereits bearbeitet' : 'Buchung bestätigen?'}
        </h1>
        {!already && (
          <>
            ...
            <form action={`/api/bookings/${booking.id}/confirm?token=${booking.token}`} method="POST">
              <button type="submit" className="w-full bg-alpine text-white font-bold py-3 rounded-xl hover:bg-alpine-light transition">
                Buchung bestätigen
              </button>
            </form>
          </>
