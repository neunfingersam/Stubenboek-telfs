...
export async function sendBookingRequestEmails(booking: {
  ...
  await resend.emails.send({
    from: 'Haus Stubenboeck <noreply@stubenboeck-telfs.com>',
    to: booking.guestEmail,
    subject: 'Ihre Buchungsanfrage — Haus Stubenboeck',
    html: `
      <div style="font-family:Georgia,serif;max-width:600px;margin:0 auto;color:#1A1A1A">
        <h1 style="color:#1C2B1E;font-size:28px">Danke fur Ihre Anfrage!</h1>
        <p>Liebe/r ${booking.guestName},</p>
        <p>wir haben Ihre Buchungsanfrage erhalten und melden uns in Kurze.</p>
        ...
        <p>Mit freundlichen Grussen,<br/>Maria Verena Stubenboeck</p>
        <hr style="border:none;border-top:1px solid #eee;margin:24px 0"/>
        <p style="font-size:12px;color:#6B7280">Haus Stubenboeck &middot; Giessenweg 9, 6410 Telfs &middot; +43 676 8303 8325</p>
      </div>
    `,
  });
  ...
  // Email to admin
  await resend.emails.send({
    from: 'Haus Stubenboeck System <noreply@stubenboeck-telfs.com>',
    to: ADMIN_EMAIL,
...
export async function sendBookingConfirmedEmail(booking: {
  ...
  await resend.emails.send({
    from: 'Haus Stubenboeck <noreply@stubenboeck-telfs.com>',
    to: booking.guestEmail,
    subject: 'Buchung bestatigt — Haus Stubenboeck',
    html: `
      <div style="font-family:Georgia,serif;max-width:600px;margin:0 auto;color:#1A1A1A">
        <h1 style="color:#1C2B1E">Ihre Buchung ist bestatigt!</h1>
        <p>Liebe/r ${booking.guestName},</p>
        <p>wir freuen uns, Ihre Buchung zu bestatigen. Wir heissen Sie herzlich willkommen!</p>
        <table style="margin:24px 0;width:100%;border-collapse:collapse">
          <tr><td style="padding:8px;color:#6B7280">Zimmer:</td><td style="padding:8px;font-weight:bold">${booking.room.name}</td></tr>
          <tr><td style="padding:8px;color:#6B7280">Anreise:</td><td style="padding:8px">${formatDate(booking.checkIn)}</td></tr>
          <tr><td style="padding:8px;color:#6B7280">Abreise:</td><td style="padding:8px">${formatDate(booking.checkOut)}</td></tr>
        </table>
        <p>Bei Fragen stehen wir gerne zur Verfugung.</p>
        <p>Mit freundlichen Grussen,<br/>Maria Verena Stubenboeck</p>
        <hr style="border:none;border-top:1px solid #eee;margin:24px 0"/>
...
export async function sendBookingRejectedEmail(booking: {
  ...
  await resend.emails.send({
    from: 'Haus Stubenboeck <noreply@stubenboeck-telfs.com>',
    to: booking.guestEmail,
    subject: 'Buchungsanfrage — Haus Stubenboeck',
    html: `
      <div style="font-family:Georgia,serif;max-width:600px;margin:0 auto;color:#1A1A1A">
        <h1 style="color:#1C2B1E">Ihre Buchungsanfrage</h1>
        <p>Liebe/r ${booking.guestName},</p>
        <p>leider mussen wir Ihre Anfrage fur den gewunschten Zeitraum (${formatDate(booking.checkIn)} &ndash; ${formatDate(booking.checkOut)}) absagen, da das Zimmer in diesem Zeitraum leider bereits belegt ist.</p>
        <p>Wir wurden uns freuen, Sie zu einem anderen Zeitpunkt begrussen zu durfen. Bitte senden Sie uns eine neue Anfrage oder kontaktieren Sie uns direkt.</p>
        <p>Mit freundlichen Grussen,<br/>Maria Verena Stubenboeck</p>
        <hr style="border:none;border-top:1px solid #eee;margin:24px 0"/>
        <p style="font-size:12px;color:#6B7280">+43 676 8303 8325 &middot; info@stubenboeck-telfs.com</p>
      </div>
